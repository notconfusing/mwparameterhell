...
for i in range(1, CORES+1):
    filenumber = i
    filedict[i] = open('/data/users/kleinm/enwiki_lumps/enwiki_lumped_' + str(filenumber) + '.xml', 'w+') #naming convention
...
for i in range(1, CORES+1):
    if not i == 1: # we don't want to append to the first  one as it'll get the text from the main dump
        filedict[i].write(initialtext)
...
for i in range(1, CORES+1):
    if not i == currfile: #the currfile already got the terminal text from the main dump
        filedict[i].write(terminaltext)
